  ...
  public:
    mdns_registration_t():
      ...
      if (service(true, existing_instance)) {
        BOOST_LOG(error) << "Unable to register Vibepollo mDNS service"sv;
        return;
      }

      BOOST_LOG(info) << "Registered Vibepollo mDNS service"sv;
    }

    ~mdns_registration_t() override {
      if (existing_instance) {
        if (service(false, existing_instance)) {
          BOOST_LOG(error) << "Unable to unregister Vibepollo mDNS service"sv;
          return;
        }

        BOOST_LOG(info) << "Unregistered Vibepollo mDNS service"sv;
      }
    }
